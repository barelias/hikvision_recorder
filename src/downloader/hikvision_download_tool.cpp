...
std::string NetDVRTime::str() {
    ...
    res << dvr_time.dwYear << "/" << (int) dvr_time.dwMonth << "/" << dvr_time.dwDay << " " << dvr_time.dwHour << ":" << dvr_time.dwMinute << ":" << dvr_time.dwSecond;
    return res.str();
}
...
std::list<VideoBlock> HikvisionDownloader::list_videos(NetDVRTime struStartTime, NetDVRTime struStopTime) {
    ...
    while (true)
    {
        ...
        if (result == NET_DVR_ISFINDING) {
            ...
            VideoBlock video_block = VideoBlock(NetDVRTime(struFileData.struStartTime), NetDVRTime(struFileData.struStopTime));
